...
void WingedMap::read(const string &nom,Winged *winged) {
  ...
  for(unsigned int i=0;i<_mesh->nbPosition();i++) {
    _vertexEdge[i].clear();
    _wvertex.push_back(winged->createVertex(_mesh->positionOBJ(i)));
  }
  ...
  for(unsigned int i=0;i<_mesh->nbFace();i++) {
    ...
    for(unsigned int j=0;j<_mesh->nbVertex(i);j++) {
      ...
      unsigned int v=_mesh->positionIndex(i,j);

      wv_begin=_wvertex[v];


      // On cherche et on crée éventuellement une nouvelle arête
      unsigned int next=_mesh->positionIndex(i,j+1);
    ...
    current0=_mesh->positionIndex(i,0);
    current1=_mesh->positionIndex(i,1);
    pred=_mesh->positionIndex(i,-1);
    succ=_mesh->positionIndex(i,2);;
    ...
    for(unsigned int j=0;j<_mesh->nbVertex(i);j++) {
      ...
      current0=current1;
      current1=succ;
      succ=_mesh->positionIndex(i,j+3);
      prede=currente;
      currente=succe;
      succe=this->findEdge(_wvertex[current1],_wvertex[succ]);
