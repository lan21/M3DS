...
void BasicMesh::initObj(const ObjLoader &obj) {
  ...
  for(unsigned int i=0;i<obj.nbFace();++i) {
    for(unsigned int j=0;j<3;++j) {
        p3d::Vector3 position = obj.positionVertex(i,j);
        _attribute.push_back(position.x());
        _attribute.push_back(position.y());
        _attribute.push_back(position.z());

         p3d::Vector3 color = obj.normalVertex(i,j);
        _attribute.push_back((color.r()+1.0)/2.0);
        _attribute.push_back((color.g()+1.0)/2.0);
        _attribute.push_back((color.b()+1.0)/2.0);
    }
  }
...
void BasicMesh::draw() {
  ...
  glDrawArrays(GL_TRIANGLES,0,_attribute.size()/2);
