...
GLApplication::GLApplication() {
  _leftPanelMenu << "PerVertex Lighting" << "PerPixel Lighting" << "Texture Transform" << "Shadow map" << "Shadow+Light" << ""
                 << "Animate" << "" << "Display Depth Texture (switch)";
  ...
  _projectorMatrix.setIdentity();
  _projectorMatrix.translate(Vector3(0,5,0));
  _projectorMatrix.rotate(20,Vector3(0,1,0));
  _projectorMatrix.translate(Vector3(0,0,4));
  _projectorMatrix.rotate(-30,Vector3(1,0,0));
...
//  glLineWidth(4); // useless from spec
  ...
  _depthTexture.bind(2);        // _depthTexture sur l'unité de texture 2
}
...
void GLApplication::update() {
  ...
  if (mouseRight()) {
    Vector3 vertical=_projectorMatrix.inverse().transformDirection(Vector3(0,1,0));
    _projectorMatrix.mul(Matrix4::fromAngleAxis(deltaMouseX()*2,vertical));
...
void GLApplication::draw() {
  ...
  p3d::projectionMatrix.setIdentity();
...
void GLApplication::drawLight() {
  ...
  p3d::modelviewMatrix.push();
  p3d::modelviewMatrix*=_projectorMatrix;
  p3d::modelviewMatrix.push();
...
void GLApplication::lightPosition() { // should be called after camera setup in modelview
  _lightPositionEye=(p3d::modelviewMatrix*_projectorMatrix).transformPoint(Vector3(0,0,0)); // the shaders require the lightPosition in Eye Coordinates
}
